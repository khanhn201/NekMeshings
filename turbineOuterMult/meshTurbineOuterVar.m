function [elements,boundaries] = meshTurbineOuterVar(left, right, shift)
    config
    elements = [];
    boundaries = [];
    ...
    for k = 1:k_outer
        for i = 1:n_segs
            ...
            p3 = [0, (i-1)/n_segs * (right-left) + left, R_far_top];
            p4 = [0, (i)/n_segs * (right-left) + left, R_far_top];
    ...
    for k = 1:k_outer
        for i = 1:n_segs
            ...
            p3 = [0, right, (n_segs-i+1)/n_segs * (R_far_top-R_far_bottom) + R_far_bottom];
            p4 = [0, right, (n_segs-i)/n_segs * (R_far_top-R_far_bottom) + R_far_bottom];
            % element(1,:) = (k_outer-k+1)*p1/k_outer + (k-1)*p4/k_outer;
            % element(2,:) = (k_outer-k)*p1/k_outer + (k)*p4/k_outer;
            % element(3,:) = (k_outer-k)*p2/k_outer + (k)*p3/k_outer;
            % element(4,:) = (k_outer-k+1)*p2/k_outer + (k-1)*p3/k_outer;
    ...
    for k = 1:k_outer
        for i = 1:n_segs
            ...
            p1 = [0, -R*cosd(45 + (i)/n_segs*90), -R*sind(45 + (i)/n_segs*90)];
            p2 = [0, -R*cosd(45 + (i-1)/n_segs*90), -R*sind(45 + (i-1)/n_segs*90)];
            p3 = [0, (i-1)/n_segs * (right-left) + left, R_far_bottom];
            p4 = [0, (i)/n_segs * (right-left) + left, R_far_bottom];
            element(1,:) = (k_outer-k+1)*p1/k_outer + (k-1)*p4/k_outer;
    ...
    for k = 1:k_outer
        for i = 1:n_segs
            ...
            p1 = [0, -R*cosd(-45 + (i)/n_segs*90), R*sind(-45 + (i)/n_segs*90)];
            p2 = [0, -R*cosd(-45 + (i-1)/n_segs*90), R*sind(-45 + (i-1)/n_segs*90)];
            p3 = [0, left, (i-1)/n_segs * (R_far_top-R_far_bottom) + R_far_bottom];
            p4 = [0, left, (i)/n_segs * (R_far_top-R_far_bottom) + R_far_bottom];
            element(1,:) = (k_outer-k+1)*p1/k_outer + (k-1)*p4/k_outer;
            element(2,:) = (k_outer-k)*p1/k_outer + (k)*p4/k_outer;
            element(3,:) = (k_outer-k)*p2/k_outer + (k)*p3/k_outer;
            element(4,:) = (k_outer-k+1)*p2/k_outer + (k-1)*p3/k_outer;
