...
function [elements,boundaries, pp_coarse, X, Y, first_layer] = meshOuterElliptic(pp, arc_length, arc_length_at_max_y)
    ...
    for k=1:k_bl
        ...
        [pptemp, arc_lengthtemp, cumulative_arc_lengthstemp, arc_length_at_max_ytemp]= fitSplineCus(layer_next, floor(k/5)+1);
        s_finetemp = cumulative_arc_lengthstemp(1:end-1);
        % diff_rate = 0.15 + k/k_bl*0.75;
        % u = cumulative_arc_lengthstemp(1:end)';
        % for j = 1:ceil(exp(k/k_bl*4)-1)
        % u_new = u;
        % for i = 2:length(u)-1
        %     u_new(i) = (u(i-1)/2 + u(i+1)/2);
        % end
        % u = u_new;
        % end
        % % s_finetemp = (1-diff_rate)*cumulative_arc_lengthstemp(1:end-1)' + diff_rate*u(1:end-1);
        ...
        ratt = 0;
        if k > 3
            ratt = (mult^(k) - 1) / (mult^(k_bl*1.5) - 1);
        end
        % ratt = (exp(k/k_bl) - 1) / (exp(1) - 1);
        % % ratt = (k/k_bl)^4;
...
function [pp, arc_length, cumulative_arc_lengths, arc_length_at_max_y]= fitSplineCus(slice, k);
