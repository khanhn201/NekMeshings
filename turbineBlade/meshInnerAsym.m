function [elements,boundaries] = meshInnerAsym(pp, arc_length, arc_length_at_max_y, flipped)
    config
    ...
    if flipped == true
        ...
        element = [];
        element(1,:) = points_top(3, :);
        element(2,:) = points_top(2, :);
        element(3,:) = (points_top(2, :)+p14)/2;
        element(4,:) = pnl;
        elements(end+1, :, :) = element;
        element = [];
        element(1,:) = points_bottom(end-1, :);
        element(2,:) = points_bottom(end-2, :);
        element(3,:) = pnl;
        element(4,:) = (points_bottom(end-1, :) + p14)/2;
        elements(end+1, :, :) = element;
        % Right tip
        element = [];
